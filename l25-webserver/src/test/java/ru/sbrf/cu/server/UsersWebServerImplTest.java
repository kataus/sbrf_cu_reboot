...
import ru.sbrf.cu.server.utils.HttpUrlConnectionHelper;
import ru.sbrf.cu.server.utils.WebServerHelper;
...
class UsersWebServerImplTest {
    ...
    void shouldReturnForbiddenStatusForUserRequestWhenUnauthorized() throws Exception {
        HttpURLConnection connection = HttpUrlConnectionHelper.sendRequest(HttpUrlConnectionHelper.buildUrl(WEB_SERVER_URL, API_USER_URL, null), HttpMethod.GET);
    ...
    void shouldReturnJSessionIdWhenLoggingInWithCorrectData() throws Exception {
        HttpCookie jSessionIdCookie = WebServerHelper.login(HttpUrlConnectionHelper.buildUrl(WEB_SERVER_URL, LOGIN_URL, null), DEFAULT_USER_LOGIN, DEFAULT_USER_PASSWORD);
    ...
    void shouldNotReturnJSessionIdWhenLoggingInWithIncorrectData() throws Exception {
        HttpCookie jSessionIdCookie = WebServerHelper.login(HttpUrlConnectionHelper.buildUrl(WEB_SERVER_URL, LOGIN_URL, null), INCORRECT_USER_LOGIN, DEFAULT_USER_PASSWORD);
    ...
    void shouldReturnCorrectUserWhenAuthorized() throws IOException {
        ...
        HttpURLConnection connection = HttpUrlConnectionHelper.sendRequest(HttpUrlConnectionHelper.buildUrl(WEB_SERVER_URL, API_USER_URL, List.of(String.valueOf(DEFAULT_USER_ID))), HttpMethod.GET);
        connection.setRequestProperty(WebServerHelper.COOKIE_HEADER, String.format("%s=%s", jSessionIdCookie.getName(), jSessionIdCookie.getValue()));
        int responseCode = connection.getResponseCode();
        String response = HttpUrlConnectionHelper.readResponseFromConnection(connection);
